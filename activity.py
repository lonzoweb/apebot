...
def format_day_activity(date_str, hourly_data, top_users):
    ...
    peak_hour = max(hourly_data.items(), key=lambda x: x[1])[0] if total_messages > 0 else 0
    
    # Build output
    lines = [
        f"📊 **Activity for {day_name}**",
        "─" * 40
    ]
...
def format_month_overview(daily_data):
    ...
    for date_str, count in daily_data:
        ...
        today_marker = " (Today)" if date_str == datetime.now().strftime('%Y-%m-%d') else ""

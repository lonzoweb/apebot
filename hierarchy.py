...
async def send_entity_details(ctx, entity_key):
    ...
    if entity.get("alt_names"):
        embed.add_field(
            name="Also Known As", value=", ".join(entity["alt_names"]), inline=False
        )
...
async def send_entity_list(ctx, page=1):
    ...
    description = "\n".join(
        [f"**{entity['name']}** ({entity['rank']})" for key, entity in entities]
    )
    ...
    embed.set_footer(
        text=f"Page {current_page}/{total_pages} • {total} total entities • .hierarchy list [page]"
    )
...
async def send_hierarchy_chart(ctx):
    ...
    for i, chunk in enumerate(chunks):
        embed = discord.Embed(
            title=f"📊 Fallen Angel Hierarchy"
        ...
        if i == len(chunks) - 1:  # Last chunk
            embed.set_footer(
                text="Use .hierarchy [name] for detailed info • .hierarchy list for full alphabetical list"
            )
        await ctx.send(embed=embed)
...
async def send_goetia_list(ctx, page=1):
    ...
    embed.set_footer(
        text=f"Page {page}/{total_pages} • {total} total spirits • .hierarchy goetia [page]"
    )
...
async def send_tradition_list(ctx, tradition):
    ...
    embed.set_footer(
        text=f"Showing {len(results)} entities • Use .hierarchy [name] for details"
    )
...
async def send_rank_list(ctx, rank):
    ...
    description = "\n".join(
        [f"**{entity['name']}**\n*{entity['domain']}*" for key, entity in results]
    )
...
def get_stats():
    ...
    # Count Goetia
    goetia_count = sum(
        1 for e in HIERARCHY_DB.values() if "Ars Goetia" in e["tradition"]
    )
...
async def send_stats(ctx):
    ...
    embed.add_field(
        name="👁️ Watchers", value=f"{stats['watchers']} documented", inline=True
    )

    # Top traditions
    top_traditions = sorted(
        stats["traditions"].items(), key=lambda x: x[1], reverse=True
    )[:5]
    traditions_text = "\n".join(
        [f"• {trad}: {count}" for trad, count in top_traditions]
    )
    embed.add_field(name="🌍 Top Traditions", value=traditions_text, inline=False)

    # Top ranks
    top_ranks = sorted(stats["ranks"].items(), key=lambda x: x[1], reverse=True)[:5]
    ranks_text = "\n".join([f"• {rank}: {count}" for rank, count in top_ranks])
    embed.add_field(name="⚔️ Top Ranks", value=ranks_text, inline=False)
...
def search_hierarchy(keyword):
    ...
    for key, entity in HIERARCHY_DB.items():
        ...
        if (
            ...
            or keyword_lower in entity["domain"].lower()
            or keyword_lower in entity["description"].lower()
            or any(keyword_lower in alt.lower() for alt in entity.get("alt_names", []))
        ):

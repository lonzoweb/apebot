...
Now with persistent scoreboard tracking
"""
...
BATTLE_CHANNEL_NAME = "forum"  # Only count reactions in this channel
...
async def on_reaction_during_battle(payload, channel):
    ...
    # Only count reactions in the battle channel (forum)
    if channel.name != BATTLE_CHANNEL_NAME:
        return
    
    # Add reaction to battle count
    battle_data.add_reaction(
        payload.message_id,
        payload.emoji,
        payload.user_id
    )
...
def format_reactions(reaction_dict, limit=10):
    ...
    sorted_reactions = sorted(reaction_dict.items(), key=lambda x: x[1], reverse=True)[:limit]
...
async def end_battle_and_announce():
    ...
    user1_reactions = battle_data.get_total_reactions(battle_data.user1.id)
    user2_reactions = battle_data.get_total_reactions(battle_data.user2.id)
    ...
    if winner:
        ...
        # Winner's reactions
        winner_reactions = battle_data.user1_reactions if winner == battle_data.user1 else battle_data.user2_reactions
        embed.add_field(
        ...
        # Loser's reactions
        loser_reactions = battle_data.user2_reactions if winner == battle_data.user1 else battle_data.user1_reactions
        embed.add_field(
...
async def start_battle(ctx, user1: discord.Member, user2: discord.Member):
    ...
    if active_battle:
        return await ctx.send("⚠️ There's a battle underway! Use `.battle stop` to end it first.")
...
async def show_scoreboard(ctx):
    ...
    embed = discord.Embed(
        title="⚔️ BATTLE SCOREBOARD",
        description="Apeiron Top Champions",
        color=discord.Color.gold()
    )
    
    scoreboard_text = []
    for i, (username, wins, total_reactions) in enumerate(scoreboard, 1):
        medal = "🥇" if i == 1 else "🥈" if i == 2 else "🥉" if i == 3 else f"{i}."
        scoreboard_text.append(f"{medal} **{username}** - {wins} wins ({total_reactions:,} reactions)")
    
    embed.description = "\n".join(scoreboard_text)
    embed.set_footer(text="React to messages during battles to vote!")
